...
- V0.4.5a (2025-10-04) - 库与链接体验  
  - 编辑器中的 [[页面链接]] 展示为不带 .md 的“笔记名”；补全候选基于 SQLite files.stem 前缀匹配，仅显示笔记名  
  - 支持从链接创建新笔记：缺失时自动在 pages/ 生成 pages/<笔记名>.md  
  - 按住 Shift 点击链接，在“新窗口”中打开编辑器（窗口名=笔记名）  
  - 内容块（{{...}}）浮窗在 IME 结束约 250ms 后稳定出现；支持“全局更新/取消”，保障写入原子性与一致性  
  - 库管理：支持添加/切换/移除库；清空并重建 .EvoNotDB；默认禁止把程序目录作为库；无库态显示欢迎语并禁用编辑
