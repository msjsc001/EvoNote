...
欢迎参与 EvoNote 的开发！本项目基于纯净的 Python 构建，并采用微内核架构。

### 技术栈 (Tech Stack)
- **核心逻辑 & GUI**: [Python 3.10+](https://www.python.org/) + [PySide6](https://doc.qt.io/qtforpython-6/) (Qt for Python).
- **Markdown 解析**: [markdown-it-py](https://github.com/executablebooks/markdown-it-py) (标准且可扩展的解析器).
- **全文检索引擎**: [Whoosh](https://whoosh.readthedocs.io/en/latest/) (纯 Python 实现的快速检索引擎).
- **文件系统监控**: [watchdog](https://github.com/gorakhargosh/watchdog) (实时响应 Markdown 文件的外部修改).

### 项目结构 (Project Structure)
```text
EvoNote/
├── main.py             # 智能引导程序 (处理依赖与入口)
├── _main_impl.py       # 实际的 PySide6 应用入口点
├── core/               # 核心微内核 (UI 管理、配置、插件注册机制)
├── plugins/            # 所有业务功能 (编辑器、文件树、搜索、大纲等均是独立插件)
├── services/           # 后台服务 (例如基于 Whoosh 的全文索引守护进程)
├── assets/             # 静态资源 (图标, QSS 主题样式表)
└── docs/ & scripts/    # 开发者文档与实用测试脚本
```

### 开发核心技术点
1. **微内核与插件化**: EvoNote 本质上只是一个包含多个插槽的窗口。通过 `core.plugin_manager` 加载 `plugins/` 目录下的各个组件并将其停靠 (Dock) 到主窗口。
2. **事件总线 (Event Bus)**: 插件间的通讯尽量避免强耦合。请依赖 Qt 的 `Signals` (定义于 `core/signals.py` 中) 广播和监听全局事件（例如 `file_opened`, `theme_changed` 等）。
3. **内容块同步 (Content Blocks)**: 基于标识符提取并在本地缓存中进行无缝跨文件同步替换的功能，是 EvoNote 数据管理的核心创新点之一。

### 构建可执行文件 (Build Executable)
如果你希望将应用打包为独立、免环境安装的 Windows `.exe` 程序：
...
2. 在项目根目录下执行打包命令 (示例)：
   ...
   ```
   *(注：由于 EvoNote 为插件化结构且包含静态资源，完整的打包可能需要修改 `.spec` 文件以使用 `--add-data` 包含 `plugins/`、`assets/` 及依赖等，详细配置请参阅 PyInstaller 官方文档。)*
