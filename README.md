...
<img alt="image" src="https://github.com/user-attachments/assets/2c4e2b39-8b13-469e-8626-93e81670ece9" />
...
> **⚠️ 开发测试阶段 (Pre-Alpha)**
>
> 由于我是用业余时间没有计划的进行本项目，它会长期很缓慢的处于**并不活跃的早期开发阶段，核心架构与功能需要迭代很多bug和需要添加功能**。
> - **请勿**将其用于管理重要或不可替代的数据。
> - 数据格式、API 和配置可能在未来版本中**发生不兼容变更**。
> - 当前版本仅供技术预览、测试反馈、二开的一个模板。
> - 请在使用前自行备份您的数据。
...
- [x] **块引与块嵌入深度编辑**：实现了源头块的双向同步，且提供**悬浮的完整子树渲染编辑模式**。
- [x] **大纲局部视图 (Zoom)**：支持点击锚点进入沉浸式的局部子树聚焦编辑，顶部带有层级面包屑导航，任何修改将精准无缝同步至源文件。
- [x] **极速无感热重载**：打通前后端心跳总线 (`evo-block-sync`)，实现毫秒级的纯粹文本到前端组件的块级双向状态热更新。
- [x] **跨页精确定位与滚动记忆**：在跳转到目标源位置时带有平滑定位闪烁特效；不同页面的滚动点状态将完全缓存记忆。
