...
本项目依赖多个第三方开源库（如 Tauri、React、BlockNote、SQLite 等），这些组件各自遵循其原始的开源许可证。本项目不对第三方组件的行为、安全性或稳定性做任何担保。

### 知识产权

本项目为独立开发的原创作品，不包含对任何商业软件的代码抄袭。项目中使用的所有第三方依赖均采用 MIT、Apache 2.0 等宽松开源许可证，使用方式符合各自的许可条款。
